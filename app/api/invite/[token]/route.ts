...
export async function PATCH(request: Request, { params }: { params: { token: string } }) {
    ...
    if (newInvite.plusOne < 0) {
        return Response.json({errorMessage: "I know you don't have many friends, but let's keep it positive."}, {status: 400})
    }
    if (newInvite.plusOne > 4) {
        return Response.json({errorMessage: `Come on that's kinda rude to bring ${newInvite.plusOne} strangers to a party.`}, {status: 400})
    }
